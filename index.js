...
const fetchCustomObjectsData = async () => {
  if (customObjectsData.length === 0) {
    try {
      const custom_objects_url = `https://api.hubapi.com/crm/v3/objects/${CUSTOM_OBJECT_TYPE}?properties=test,name,details,amount`;
      const headers = {
...
app.post("/update-cobj", async (req, res) => {
  ...
  try {
    ...
    if (index !== -1) {
      customObjectsData[index].properties.test = req.body.test;
      customObjectsData[index].properties.name = req.body.name;
